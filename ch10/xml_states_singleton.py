"""
Demonstration of the state pattern implemented using the singleton pattern.

Module variables can be used to mimic singletons. This is not as safe as a true
singleton because the variable can be reassigned. To use module level variables
instead of singletons, we can instantiate an instance of the class after it is
defined. Instead of creating a new object every time we change states, we can
...
class FirstTag:
    ...
    def process(self, remaining_string, parser):
        ...
        parser.state = child_node       # use singleton module level variable
...
class ChildNode:
    ...
    def process(self, remaining_string, parser):
        ...
        if stripped.startswith('</'):
            ...
            parser.state = close_tag    # use singleton module level variable
        elif stripped.startswith('<'):
            # parser.state = OpenTag()
            parser.state = open_tag     # use singleton module level variable
        else:
            # parser.state = TextNode()
            parser.state = text_node    # use singleton module level variable
...
# Module level variables used to mimic singletons
# This enables us to reuse the same objects instead of defining new objects
# every time that must be garbage collected.
first_tag = FirstTag()
